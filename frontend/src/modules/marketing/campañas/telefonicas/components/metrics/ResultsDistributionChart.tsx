...
export const ResultsDistributionChart: React.FC<ResultsDistributionChartProps> = ({ data }) => {
    ...
    const isAnimated = useAnimateOnMount(100);
    ...
    return (
        <div className="flex flex-col gap-2 rounded-lg bg-white p-6 border border-gray-200/50 shadow-sm">
                ...
                <div className="relative w-40 h-40">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                        ...
                        {segments.map((segment, index) => (
                            <circle
                                key={index}
                                className={`stroke-current transition-all duration-1000 ease-out ${segment.resultado === 'CONTACTADO' || segment.resultado === 'INTERESADO'
                                    ? 'text-green-500'
                                ...
                                strokeDashoffset={isAnimated ? segment.offset : 100}
                                strokeLinecap="round"
            ...
            <div className={`grid grid-cols-2 gap-x-4 gap-y-2 transition-opacity duration-700 ${isAnimated ? 'opacity-100' : 'opacity-0'}`}>
