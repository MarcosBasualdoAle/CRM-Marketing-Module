...
export const guionesApi = {
    ...
    /**
     * GET /api/v1/campanias-telefonicas/{id}/guiones-estructurados
     * Lista los guiones estructurados de una campaña específica
     */
    listGuionesByCampaign: async (idCampania: number): Promise<GuionDTO[]> => {
        const response = await apiClient.get(`${API_BASE_URL}/campanias-telefonicas/${idCampania}/guiones-estructurados`);
        return response.data.data;
    },
