...
import { registerCacheInvalidator, setupCacheInvalidationInterceptor } from '../interceptors/cacheInvalidationInterceptor';
import type {
    CampaniaTelefonica,
    Contacto,
    Llamada,
...
// Tiempo de vida del caché antes de considerarse "stale" (5 minutos)
// Datos más frescos que esto no se recargarán automáticamente
...
export const CampaignCacheProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const getCachedData = useCallback(async <T,>(
        ...
        const cachedData = cacheRef.current[campaignId]?.[dataType];
        const lastUpdated = cacheRef.current[campaignId]?.lastUpdated[dataType];

        // Si tenemos datos y son frescos (menos de STALE_TIME), retornarlos
        if (cachedData !== undefined && lastUpdated) {
            const age = Date.now() - lastUpdated.getTime();
            if (age < STALE_TIME) {
                logger.hit(`${dataType} for campaign ${campaignId} (age: ${Math.round(age / 1000)}s)`);
    ...
    // Nueva función para invalidación externa (desde interceptor)
    const invalidateCacheExternal = useCallback((
        campaignId: number | null,
        dataTypes: string[],
        affectsGlobalList?: boolean
    ) => {
        if (campaignId) {
            invalidateCache(campaignId, dataTypes as CacheDataType[]);
        }

        if (affectsGlobalList) {
            // Llamar a CampaignsContext para refrescar lista
            // Esto se hace mediante custom event
            window.dispatchEvent(new CustomEvent('invalidate-campaign-list'));
        }
    }, [invalidateCache]);

    // Registrar invalidador al montar
    useEffect(() => {
        registerCacheInvalidator(invalidateCacheExternal);
        setupCacheInvalidationInterceptor();

        return () => {
            // Cleanup si es necesario (actualmente no hay cleanup para el interceptor)
        };
