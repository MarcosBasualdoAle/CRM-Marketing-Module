...
export const CampaignsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // Escuchar evento de invalidación global de lista de campañas
    React.useEffect(() => {
        const handleInvalidate = () => {
            console.log('[CampaignsContext] Global campaign list invalidated');
            setHasLoaded(false);
            fetchCampanias(true);
        };

        window.addEventListener('invalidate-campaign-list', handleInvalidate);

        return () => {
            window.removeEventListener('invalidate-campaign-list', handleInvalidate);
        };
    }, [fetchCampanias]);
