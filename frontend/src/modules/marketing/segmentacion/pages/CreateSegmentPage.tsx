...
export const CreateSegmentPage: React.FC = () => {
        ...
        const fetchDetails = async () => {
            ...
            try {
                ...
                if (tipoAudiencia === 'CLIENTE') {
                    // Llamar al endpoint de clientes batch
                    const response = await fetch('/api/v1/internal/clientes/batch', {
                        method: 'POST',
                        headers: {
                    ...
                    // Transform client data to preview format
                    const members = clientes.map((cliente: any) => ({
                        id: cliente.id,
    ...
    useEffect(() => {
        const fetchFilters = async () => {
            ...
            // Para CLIENTE: usar filtros basados en la API real de Ventas
            try {
                const clientFilters: FilterOption[] = [
                    { value: 'edad', label: 'Edad', type: 'number' },
                    { value: 'categoria', label: 'Categoría', type: 'select', options: ['VIP', 'Estándar'] },
                    { value: 'estado', label: 'Estado', type: 'select', options: ['ACTIVO', 'INACTIVO'] },
                    { value: 'recencyScore', label: 'Score de Recencia (RFM)', type: 'number' },
                    { value: 'frequencyScore', label: 'Score de Frecuencia (RFM)', type: 'number' },
                    { value: 'monetaryScore', label: 'Score Monetario (RFM)', type: 'number' },
                    { value: 'ubicacion', label: 'Ubicación', type: 'text' },
                ];

                setAvailableFilters(clientFilters);
                setFilters([]); // Clear existing filters when audience changes
            } catch (error) {
                console.error('Error setting client filters:', error);
                setAvailableFilters([]);
            }
        };
