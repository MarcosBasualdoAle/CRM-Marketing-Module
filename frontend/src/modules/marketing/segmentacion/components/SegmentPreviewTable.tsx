            ...
            <div className="overflow-x-auto max-h-[70vh] overflow-y-auto">
                <table className="min-w-full">
                    <thead className="bg-table-header sticky top-0 z-10">
                        <tr>
                            <th className="p-4 text-sm font-semibold text-dark tracking-wide text-left">
                                Nombre
                            </th>
                            <th className="p-4 text-sm font-semibold text-dark tracking-wide text-left w-20">
                                Edad
                            </th>
                            <th className="p-4 text-sm font-semibold text-dark tracking-wide text-left">
                                Correo
                            </th>
                            <th className="p-4 text-sm font-semibold text-dark tracking-wide text-left w-32">
                                Número
                            </th>
                        </tr>
                    </thead>
                        ...
                        {filteredMembers.length === 0 ? (
                            ...
                            filteredMembers.map((member) => (
                                <tr key={member.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100">
                                    <td className="px-4 py-3 text-sm text-gray-800">{member.nombre || 'No especificado'}</td>
                                    <td className="px-4 py-3 text-sm text-gray-800">{member.edad === 0 || !member.edad ? 'No especificada' : member.edad}</td>
                                    <td className="px-4 py-3 text-sm text-gray-800">{member.correo || 'No especificado'}</td>
                                    <td className="px-4 py-3 text-sm text-gray-800">{member.telefono || 'No especificado'}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
